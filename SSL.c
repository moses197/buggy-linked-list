    ...
    while (current != NULL)
    {
        if (current->data == data)
        {
            ...
            if (current == head)
            {
                ...
                free(current); // you couldn't belive i didn't free. BAD memory fucking leak
                current = NULL; // and i didn't set this too to null so that i won't have access to it again
            }
            return 1;
        }
        prev = current;
        ...
        if (num_received == 1 && option > 0 && option <= 5)
        {
            switch(option) {
                ...
                case 3:
                    ...
                    scanf("%d", &option2);
                    Node *neww = insertNode(option, option2);
                    if (neww == NULL)
